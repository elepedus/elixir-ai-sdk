    ...
    test "connects to local LMStudio and streams text" do
      ...
      case :httpc.request(:get, {"#{lmstudio_url}/v1/models", []}, [], []) do
        {:ok, {{_, 200, _}, _, _}} ->
          ...
          model_id = "gemma-3-4b-it-qat"  # This model is in the list from LMStudio
          ...
          model = AI.openai_compatible(model_id, 
          ...
          result = AI.stream_text(%{
            ...
            prompt: "Say hello",
            max_tokens: 20,  # Minimal tokens for faster response
            temperature: 0.7
          })
          ...
          chunks = Enum.reduce_while(response.stream, [], fn
          ...
          result_non_streaming = AI.generate_text(%{
             model: model,
             system: "You are a helpful assistant.",
             prompt: "Say hello",
             max_tokens: 20,
          ...
          error_chunks = Enum.filter(chunks || [], fn
            {:error, _} -> true
            _ -> false
          end)
          ...
          # Get text chunks
          text_chunks = Enum.filter(chunks || [], fn
            {:text_delta, _} -> true
            _ -> false
          end)
          ...
          if match?({:ok, _}, result_non_streaming) do
            ...
            IO.puts("WARNING: Even non-streaming API call failed. LMStudio may not be configured correctly.")
