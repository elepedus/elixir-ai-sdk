defmodule AI.StreamingImplementationsComparisonTest do
  ...
  @model "gpt-4o-mini"
  @temperature 0.0  # Using 0 temperature for more deterministic output
  @max_tokens 100
  
  describe "Comparison of streaming implementations" do
    @tag :e2e
    test "compare outputs from openai_ex streaming and AI.stream_text" do
      ...
      similarity_ratio = if openai_ex_content != "" and ai_sdk_content != "" do
  ...
  defp analyze_chunk_patterns(openai_ex_chunks, ai_sdk_chunks) do
    ...
    # Average chunk sizes
    openai_ex_avg_size = if openai_ex_chunk_count > 0 do
    ...
    ai_sdk_avg_size = if ai_sdk_chunk_count > 0 do
    ...
    if openai_ex_chunk_count > 0 do
      ...
      openai_ex_chunks |> Enum.take(sample_count) |> Enum.with_index(1) |> Enum.each(fn {chunk, i} ->
    ...
    if ai_sdk_chunk_count > 0 do
      ...
      ai_sdk_text_chunks |> Enum.take(sample_count) |> Enum.with_index(1) |> Enum.each(fn {chunk, i} ->
        IO.puts("  #{i}. \"#{chunk}\" (#{String.length(chunk)} chars)")
      end)
    end
  end
  ...
  defp calculate_levenshtein_distance(s, t) do
    ...
    cond do
      s == t -> 0
      String.length(s) == 0 -> String.length(t)
      String.length(t) == 0 -> String.length(s)
      true ->
  ...
  defp initialize_matrix(s_chars, t_chars) do
    ...
    row0 = Enum.reduce(0..t_len, %{}, fn j, acc ->
      Map.put(acc, {0, j}, j)
    end)
